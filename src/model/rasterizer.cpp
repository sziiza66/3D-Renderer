...
inline bool IsTriangleInTheFrustum(const Renderer3D::Kernel::Triangle& triangle) {
    return abs(triangle.matrix(2, 0)) < 1 && abs(triangle.matrix(2, 1)) < 1 && abs(triangle.matrix(2, 2)) < 1;
}
...
std::tuple<uint8_t, uint8_t, uint8_t> GetVerticalOrderOfVertices(const Renderer3D::Kernel::Triangle& triangle) {
    ...
    if (triangle.matrix(0, std::get<2>(ret)) < triangle.matrix(0, std::get<1>(ret))) {
        std::swap(std::get<2>(ret), std::get<1>(ret));
    }
    if (triangle.matrix(0, std::get<2>(ret)) < triangle.matrix(0, std::get<0>(ret))) {
        std::swap(std::get<2>(ret), std::get<0>(ret));
    }
    if (triangle.matrix(0, std::get<1>(ret)) < triangle.matrix(0, std::get<0>(ret))) {
...
void DrawTriangle(Frame& frame, std::vector<double>& z_buffer_, const Triangle& triangle) {
    ...
    Eigen::Vector3d v1 = triangle.matrix.col(middle).head(3) - triangle.matrix.col(lowest).head(3);
    Eigen::Vector3d v2 = triangle.matrix.col(highest).head(3) - triangle.matrix.col(lowest).head(3);
